...
class GraphDataset(Dataset):
    def __init__(self, G_comm_list, S_comm_list, G_sens_list,
                 ...
                 N_RF=4, K_tx=6, K_rx=6, K_tx_milp=2, K_rx_milp=2):
        self.Gc = G_comm_list
        self.Sc = S_comm_list
        self.Gs = G_sens_list
        ...
        self.K_tx_milp, self.K_rx_milp = K_tx_milp, K_rx_milp
    ...
    def __getitem__(self, i):
        inst = AssentData(G_comm=self.Gc[i], S_comm=self.Sc[i], G_sens=self.Gs[i],
                          ...
                          solution=self.sol[i], N_RF=self.N_RF, K_tx=self.K_tx, K_rx=self.K_rx,
                          device=self.device, K_tx_milp=self.K_tx_milp, K_rx_milp=self.K_rx_milp,)
        ...
        # y_tx labels on ('ap','senses_tx','target')
        ytx_mat = self.sol[i].y_tx  # [A,T] binary
        ei_tx = data[('ap', 'senses_tx', 'target')].edge_index.numpy()
        a_tx, t_tx = ei_tx[0], ei_tx[1]
        ytx = np.asarray(ytx_mat, np.float32)[a_tx, t_tx].reshape(-1, 1)
        data[('ap', 'senses_tx', 'target')].y = torch.from_numpy(ytx)

        # y_rx labels on ('ap','senses_rx','target')
        yrx_mat = self.sol[i].y_rx  # [A,T]
        ei_rx = data[('ap', 'senses_rx', 'target')].edge_index.numpy()
        a_rx, t_rx = ei_rx[0], ei_rx[1]
        yrx = np.asarray(yrx_mat, np.float32)[a_rx, t_rx].reshape(-1, 1)
        data[('ap', 'senses_rx', 'target')].y = torch.from_numpy(yrx)
