...
class AssentData:
    ...
    K_tx: int = 6           # top-k for tx-side features
    K_rx: int = 6           # top-k for rx-side features
    device: str = "cpu"
    K_tx_milp: int = 2      # MILP target tx cap
    K_rx_milp: int = 2      # MILP target rx cap
...
def data_loader(nsamps=10000, CONSOLE_RUN=False):
    ...
    lib.print_log(tag='LOAD', message=f"Loaded exp1 results from '{exp1_folder_path}'")
    ...
    metadata_path = os.path.join(exp2_folder_path, 'metadata.json')
    with open(metadata_path) as f:
