...
rid_ext = ''   # run_id extension {'' or 'a'}

#%% Loading data

# NNConv data
run_id = f'run_01{rid_ext}'
hist_filename       = '2025-11-08-14-52_history.json' if run_id == 'run_01a' else '2025-11-08-05-20_history.json'
summary_filename    = '2025-11-08-14-52_summary.json' if run_id == 'run_01a' else '2025-11-08-05-20_summary.json'
...
run_id = f'run_02{rid_ext}'
hist_filename       = '2025-11-08-16-55_history.json' if run_id == 'run_02a' else '2025-11-07-20-41_history.json'
summary_filename    = '2025-11-08-16-55_summary.json' if run_id == 'run_02a' else '2025-11-07-20-41_summary.json'
hist_path = os.path.join(data_dir, run_id, hist_filename)
summary_path = os.path.join(data_dir, run_id, summary_filename)
...
# GATv2Conv data
run_id = f'run_03{rid_ext}'
hist_filename       = '2025-11-08-21-11_history.json' if run_id == 'run_03a' else '2025-11-07-23-36_history.json'
summary_filename    = '2025-11-08-21-11_summary.json' if run_id == 'run_03a' else '2025-11-07-23-36_summary.json'
hist_path = os.path.join(data_dir, run_id, hist_filename)
summary_path = os.path.join(data_dir, run_id, summary_filename)
...
nsamps = metadata_nnconv['config']['nsamps']

results = input_data_loader(path_to_input_data=metadata_nnconv['config']['path_to_input_data'], input_file_name=metadata_nnconv['config']['input_file_name'],
                            path_to_solution_data=metadata_nnconv['config']['path_to_solution_data'], solution_file_name=metadata_nnconv['config']['solution_file_name'],
                            input_parts_to_load=metadata_nnconv['config']['input_parts_to_load'], solution_parts_to_load=metadata_nnconv['config']['solution_parts_to_load'],
                            nsamps=nsamps)
...
nnconv_pareto_id = 0 if run_id == 'run_01a' else 1
nnconv_pareto_epoch = 38 if run_id == 'run_01a' else 32
thr_dict_nnconv = {
...
model_path = os.path.join(data_dir, run_id, f"pareto/pareto_ep0{nnconv_pareto_epoch}.pt")
...
nsamps = metadata_transformer['config']['nsamps']

results = input_data_loader(path_to_input_data=metadata_transformer['config']['path_to_input_data'], input_file_name=metadata_transformer['config']['input_file_name'],
                            path_to_solution_data=metadata_transformer['config']['path_to_solution_data'], solution_file_name=metadata_transformer['config']['solution_file_name'],
                            input_parts_to_load=metadata_transformer['config']['input_parts_to_load'], solution_parts_to_load=metadata_transformer['config']['solution_parts_to_load'],
                            nsamps=nsamps)
...
transformer_pareto_id = 0 if run_id == 'run_02a' else 0
transformer_pareto_epoch = 61 if run_id == 'run_02a' else 70
...
model_path = os.path.join(data_dir, run_id, f"pareto/pareto_ep0{transformer_pareto_epoch}.pt")
state = torch.load(model_path, map_location=device)
...
nsamps = metadata_gat['config']['nsamps']

results = input_data_loader(path_to_input_data=metadata_gat['config']['path_to_input_data'], input_file_name=metadata_gat['config']['input_file_name'],
                            path_to_solution_data=metadata_gat['config']['path_to_solution_data'], solution_file_name=metadata_gat['config']['solution_file_name'],
                            input_parts_to_load=metadata_gat['config']['input_parts_to_load'], solution_parts_to_load=metadata_gat['config']['solution_parts_to_load'],
                            nsamps=nsamps)
...
gat_pareto_id = 0 if run_id == 'run_03a' else 0
gat_pareto_epoch = 64 if run_id == 'run_03a' else 91
thr_dict_gat = {
...
model_path = os.path.join(data_dir, run_id, f"pareto/pareto_ep0{gat_pareto_epoch}.pt")
state = torch.load(model_path, map_location=device)
...
if show_lr:
    lr_handle = [Line2D([0],[0], color=colors['lr'], lw=lw_lr, linestyle=(0,(1,1)), label="Learning rate (right)")]
    ax1.legend(handles=lr_handle, frameon=True, loc="lower right")
ax1.set_xlim(0, 90)
...
if SAVE_FIG:
    ...
    ext = rid_ext if rid_ext == '' else f'-{rid_ext}'
    filename = os.path.join(save_path, timestamp + f"_loss_and_objVal{ext}.png")
    fig.savefig(filename, dpi=300, bbox_inches='tight', transparent=False)
    filename = os.path.join(save_path, timestamp + f"_loss_and_objVal{ext}.pdf")
    fig.savefig(filename, dpi=300, bbox_inches='tight', transparent=False)
