...
# exp1_pareto.py:
# Reading {G_comm, S_comm, G_sens} data from run_03 and running the MILP for alpha values from 0.0 to 1.0
...
width = 50
print("=" * width)
print(">> Simulation for 'exp1_pareto' Started <<".center(width))
print(f'{now.strftime("%A, %B %d, %Y at %I:%M:%S %p")}')
print("=" * width)

# Load metadata
experiment = 'exp1_pareto'
...
lib.print_log(tag='LOAD', message=f"Loaded current (exp1_pareto) metadata from '{metadata_path}'")
...
lib.print_log(tag='INFO', message=f"\"Results from exp1_baseline will be used as input for exp1_pareto\"")
...
total_iters = len(results) * len(alpha_list)
...
for alpha_idx, alpha_value in enumerate(alpha_list):
    for it in range(input_nsamps):
        current_iter = (alpha_idx * input_nsamps + it + 1)
        if (it + 1) % 10 == 0:
            print(f"[RUN] [{current_iter}/{total_iters}] Running for alpha {alpha_idx+1}/{len(alpha_list)}, data sample {it + 1}/{input_nsamps} ...")
        ...
        loop_result = {'alpha': alpha_value, 'opt_status': opt_status, 'opt_objVal': opt_objVal, 'solution': solution}
...
print("=" * width)
print(">> Simulation for 'exp1_pareto' finished <<".center(width))
